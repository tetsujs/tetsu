...
createApp({ hooks: [requestId(), accessLog()], routes });
```
...
the compiler checks that something provides it.
...
accessLog({
  write: (record) => {
    ...
    latency.labels(record.method, route).observe(record.durationMs ?? 0);
  },
});
```
